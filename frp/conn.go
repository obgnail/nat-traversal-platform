...
func (c *Conn) SendMessage(msg *Message) (err error) {
	if msg.Type == "" {
		return SendMessageError(msg)
	}
...
func (c *Conn) ReadMessage() (message *Message, err error) {
	msgBytes, err := c.Read()
	if err != nil {
		return
	}
	...
	if err = json.Unmarshal(msgBytes, message); err != nil {
		err = UnmarshalMessageError(string(msgBytes))
		return
	}

	if message.Type == "" {
		err = NotFoundError(ModelMessage, ErrArgType)
		return
	}
	return
}
...
func TryGetFreePort(retry int) (int, error) {
	...
	return 0, fmt.Errorf("retry too much time")
}
...
func Dial(host string, port int64) (conn *Conn, err error) {
	...
	log.Info("start connect:", tcpConn.LocalAddr(), "->", tcpConn.RemoteAddr())
